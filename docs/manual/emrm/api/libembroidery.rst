...
The source code is stored in the ``src/`` directory, the other
versions are considered hand-translations of this version. For example,
the ``embedded/`` directory has assembly macros that imitate the behaviour
of these functions and are documented in the Assembly Macros section.

Style
-----

Note that all new defines start with ``EMB_``, all functions with
``emb`` and all
typedefs with ``Emb`` to protect against namespace pollution.
(INCOMPLETE)
...
label                     value
========================= ======================
``EMB_LIB_VERSION_MAJOR`` 1
``EMB_LIB_VERSION_MINOR`` 0
``EMB_LIB_VERSION_PATCH`` 0
``EMB_LIB_VERSION_TAG``   "alpha"
========================= ======================
